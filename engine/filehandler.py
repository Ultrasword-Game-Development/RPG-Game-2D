...
class ChunkSaver:
    ...
    def save_chunk(cls, chunk, filepath):
        ...
        with open(filepath, 'wb') as file:
            data = {"pos": chunk.world_chunk_tile, "tilemap": [[chunk.tilemap[y][x].to_dict() for x in range(singleton.TILEMAP_WIDTH)] for y in range(singleton.TILEMAP_HEIGHT)]}
    ...
    def load_chunk(cls, filepath):
        ...
        for x in range(singleton.TILEMAP_WIDTH):
            for y in range(singleton.TILEMAP_HEIGHT):
